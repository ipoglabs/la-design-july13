...
export default function HealthBeautyForm() {
  ...
  const productType = (store as any).productType ?? "";
  ...
  useEffect(() => {
    ...
    if (!subcategory) setField("subcategory", "Health & Beauty");
  ...
  const dispatchValidated = (ok: boolean) => {
    ...
    window.dispatchEvent(new CustomEvent("healthform:validated", { detail: { ok } }));
  };
  ...
  const onSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!productType.trim()) mapped.productType = "Product type required";
