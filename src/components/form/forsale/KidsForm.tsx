...
export default function BabyKidsForm() {
  ...
  useEffect(() => {
    ...
    if (!subcategory) setField("subcategory", "Baby & Kids");
  ...
  const dispatchValidated = (ok: boolean) => {
    ...
    window.dispatchEvent(new CustomEvent("kidsform:validated", { detail: { ok } }));
  };
  ...
  return (
    ...
    >
      ...
      <ToggleButtonGroup title="Condition" singleSelect value={condition ? [condition] : []} onChange={(v) => setField("condition", v[0] ?? "")}>
        ...
        <ToggleGroupButton value="good">Good</ToggleGroupButton>
        <ToggleGroupButton value="fair">Fair</ToggleGroupButton>
      </ToggleButtonGroup>

      <ToggleButtonGroup title="Suitable Age Range" singleSelect value={ageRange ? [ageRange] : []} onChange={(v) => setField("ageRange", v[0] ?? "")}>
        <ToggleGroupButton value="0-6-months">0–6 Months</ToggleGroupButton>
        <ToggleGroupButton value="6-12-months">6–12 Months</ToggleGroupButton>
        <ToggleGroupButton value="1-3-years">1–3 Years</ToggleGroupButton>
        <ToggleGroupButton value="3-5-years">3–5 Years</ToggleGroupButton>
        <ToggleGroupButton value="5-plus">5+ Years</ToggleGroupButton>
      </ToggleButtonGroup>
