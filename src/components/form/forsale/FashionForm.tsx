...
export default function FashionAccessoriesForm() {
  ...
  useEffect(() => {
    ...
    if (!subcategory) setField("subcategory", "Fashion & Accessories");
  ...
  const dispatchValidated = (ok: boolean) => {
    ...
    window.dispatchEvent(new CustomEvent("fashionform:validated", { detail: { ok } }));
  };
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <FormField label="Size" field="size" value={size} onChange={(v) => setField("size", v)} />
      </div>
      ...
      <ToggleButtonGroup title="Gender" singleSelect value={gender ? [gender] : []} onChange={(v) => setField("gender", v[0] ?? "")}>
        ...
        <ToggleGroupButton value="unisex">Unisex</ToggleGroupButton>
      </ToggleButtonGroup>
