...
export default function ElectronicsSaleForm() {
  ...
  useEffect(() => {
    ...
    if (!subcategory) setField("subcategory", "Electronics & Gadgets");
  ...
  const dispatchValidated = (ok: boolean) => {
    ...
    window.dispatchEvent(new CustomEvent("electronicsform:validated", { detail: { ok } }));
  };
  ...
  return (
    ...
    >
      ...
      <ToggleButtonGroup title="Condition" singleSelect value={condition ? [condition] : []} onChange={(v) => setField("condition", v[0] ?? "")}>
        ...
        <ToggleGroupButton value="for-parts">For Parts</ToggleGroupButton>
      </ToggleButtonGroup>

      <ToggleButtonGroup title="Warranty" singleSelect value={warranty ? [warranty] : []} onChange={(v) => setField("warranty", v[0] ?? "")}>
        <ToggleGroupButton value="no">No Warranty</ToggleGroupButton>
        <ToggleGroupButton value="under-warranty">Under Warranty</ToggleGroupButton>
        <ToggleGroupButton value="extended">Extended Warranty</ToggleGroupButton>
      </ToggleButtonGroup>
