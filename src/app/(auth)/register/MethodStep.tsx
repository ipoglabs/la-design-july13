...
/**
 * MethodStep — Register Step 1 · Passwordless sign-up entry
 *
 * Ported from la-design-july16 (`md/feature-spec-doc/register-journey.md`),
 * adapted to la-staging's REAL backend — see login/MethodStep.tsx header
 * for the full rationale on Google/Apple wiring; same approach here:
...
export function MethodStep() {
  ...
  const storeMethod = useOnboardingStore((s) => s.method);

  const [identifierMethod, setIdentifierMethod] = useState<"email" | "phone">(
    storeMethod === "phone_otp" ? "phone" : "email"
  );
  const [email, setEmail] = useState(storeMethod === "magic_link" ? storeIdentifier ?? "" : "");
  const [phone, setPhone] = useState(storeMethod === "phone_otp" ? storeIdentifier ?? "" : "");
    ...
    <div className="min-h-screen w-full flex items-center justify-center bg-[url('/bg-market-place-vintage.png')] bg-cover bg-center bg-no-repeat px-4 py-8">
      <LaCard className="w-full max-w-xs rounded-2xl p-6 flex flex-col gap-4 shadow-[0_0_12px_rgba(0,0,0,0.45)]">
        ...
        <div className="flex flex-col gap-1">
          <h1 className="text-base font-semibold text-slate-700 tracking-tight">Join LokalAds</h1>
        ...
        <fieldset aria-label="Sign-up method" className="flex gap-5">
          <LaRadio
