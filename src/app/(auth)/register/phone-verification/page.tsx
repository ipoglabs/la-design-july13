...
export default function PhoneVerificationPage() {
  ...
  const [verified, setVerified] = useState<VerifiedEntry[]>(() => {
    ...
    const parse = (full: string | undefined, label: (typeof LABELS)[number]): VerifiedEntry | null => {
      if (!full) return null;
  ...
  async function saveVerifiedPhone(fullPhone: string) {
    const store = useRegisterStore.getState();
    const existing = store.phones;

    if (!existing.primaryNumber) {
      store.updatePhones({ primaryNumber: fullPhone });
    } else if (!existing.secondaryNumber1) {
      store.updatePhones({ secondaryNumber1: fullPhone });
    } else if (!existing.secondaryNumber2) {
      store.updatePhones({ secondaryNumber2: fullPhone });
    }
  ...
  async function handleOtpComplete(otp: string) {
    ...
    try {
      ...
      if (res.ok) {
        await saveVerifiedPhone(fullPhone);
        setVerified((prev) => [...prev, { country, phone, label: LABELS[prev.length] }]);
        toast.success("Phone verified ✅");
        setStage("summary");
      } else {
        setOtpError(true);
        setOtpErrorMsg(data?.error || "Incorrect code. Try again.");
      }
    } catch {
      setVerifying(false);
      setOtpError(true);
  ...
  function handleDelete(index: number) {
    ...
    setVerified(updated);
    setConfirmingDelete(null);
    if (updated.length === 0 && stage === "summary") setStage("enter-phone");
  }
